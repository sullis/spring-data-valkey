...
WARNING: Writing objects to a Valkey hash deletes the content from the hash and re-creates the whole hash, so data that has not been mapped is lost.
...
.Configuring a custom `ValkeyTypeMapper` via Spring Java Config
====
[source,java]
----
