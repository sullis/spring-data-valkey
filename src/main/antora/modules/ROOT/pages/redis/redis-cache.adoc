...
Spring Data Valkey provides an implementation of Spring Framework's {spring-framework-docs}/integration.html#cache[Cache Abstraction] in the `org.springframework.data.redis.cache` package.
To use Valkey as a backing implementation, add javadoc:org.springframework.data.redis.cache.ValkeyCacheManager[] to your configuration, as follows:
...
`ValkeyCacheManager` behavior can be configured with javadoc:org.springframework.data.redis.cache.ValkeyCacheManager$ValkeyCacheManagerBuilder[], letting you set the default javadoc:org.springframework.data.redis.cache.ValkeyCacheManager[], transaction behavior, and predefined caches.
...
The behavior of javadoc:org.springframework.data.redis.cache.ValkeyCache[] created by javadoc:org.springframework.data.redis.cache.ValkeyCacheManager[] is defined with `ValkeyCacheConfiguration`.
...
javadoc:org.springframework.data.redis.cache.ValkeyCacheManager[] defaults to a lock-free javadoc:org.springframework.data.redis.cache.ValkeyCacheWriter[] for reading and writing binary values.
