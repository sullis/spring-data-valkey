...
import static org.springframework.data.redis.connection.lettuce.LettuceReactiveValkeyConnection.AsyncConnect.State.*;
...
class LettuceReactiveValkeyConnection implements ReactiveValkeyConnection {
	...
	static final ValkeyCodec<ByteBuffer, ByteBuffer> CODEC = ByteBufferCodec.INSTANCE;

	private final Object mutex = new Object();

	private final AsyncConnect<StatefulConnection<ByteBuffer, ByteBuffer>> dedicatedConnection;
	private final AsyncConnect<StatefulValkeyPubSubConnection<ByteBuffer, ByteBuffer>> pubSubConnection;
	...
	LettuceReactiveValkeyConnection(LettuceConnectionProvider connectionProvider) {
		...
		this.pubSubConnection = new AsyncConnect(connectionProvider, StatefulValkeyPubSubConnection.class);
	}
	...
	public Mono<String> ping() {
		return execute(BaseValkeyReactiveCommands::ping).next();
	}
	...
	protected Mono<StatefulValkeyPubSubConnection<ByteBuffer, ByteBuffer>> getPubSubConnection() {
		return pubSubConnection.getConnection().onErrorMap(translateException());
	}

	protected Mono<? extends ValkeyClusterReactiveCommands<ByteBuffer, ByteBuffer>> getCommands() {
	...
	protected Mono<? extends ValkeyClusterReactiveCommands<ByteBuffer, ByteBuffer>> getDedicatedCommands() {
		return dedicatedConnection.getConnection().map(LettuceReactiveValkeyConnection::getValkeyClusterReactiveCommands);
	}

	private static ValkeyClusterReactiveCommands<ByteBuffer, ByteBuffer> getValkeyClusterReactiveCommands(
			StatefulConnection<ByteBuffer, ByteBuffer> connection) {

		if (connection instanceof StatefulValkeyConnection) {
			return ((StatefulValkeyConnection<ByteBuffer, ByteBuffer>) connection).reactive();
		} else if (connection instanceof StatefulValkeyClusterConnection) {
			return ((StatefulValkeyClusterConnection<ByteBuffer, ByteBuffer>) connection).reactive();
		}
	...
	interface LettuceReactiveCallback<T> {
		Publisher<T> doWithCommands(ValkeyClusterReactiveCommands<ByteBuffer, ByteBuffer> cmd);
	}

	enum ByteBufferCodec implements ValkeyCodec<ByteBuffer, ByteBuffer> {
